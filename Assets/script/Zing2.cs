...
public class Zing2 : MonoBehaviour {
    ...
    void Start ()
    {
        player = GameObject.Find("QR").GetComponent<RawImage>();
    ...
    void Update()
    {
        QRCodeString = get_xml.url;
        Color32[] color32 = useEncode(QRCodeString, 256, 256);//儲存產生的QR Code
        encoded.SetPixels32(color32);//設定要顯示的圖片像素
        encoded.Apply();//申請顯示圖片
        player.texture = encoded;
    }
    ...
    {
        //將QR Code繪製到畫面上
        GUI.DrawTexture(new Rect(600, 475, 100, 100), encoded);
    }*/
}
